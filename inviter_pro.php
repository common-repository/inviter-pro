			...
			for($t=0;$t<count($to_userid_arr);$t++){
				...
				if($to_userid)
				{
					...
					if(function_exists('bp_notifications_add_notification')){
						bp_notifications_add_notification($topic_id, $to_userid, 'inviter', $current_user_id.'_sentnotification');
					}else{
   ...
   $notification_type = $component_action_arr[1];
	...
	if($topic_details){
		...
		{ 
			...
			$post_author = $topic_post->item_id;
		}else{
			$topic_link = bbp_get_topic_permalink( $item_id );
			$post_author = bbp_get_reply_author( $item_id );
		}
